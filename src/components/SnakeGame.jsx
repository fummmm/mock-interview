...
export default function SnakeGame({ initialDir = 'RIGHT', onClose }) {
  ...
  const init = useCallback((startDir) => {
    ...
    stateRef.current = { snake, food, dir: d, nextDir: d, alive: true, started: false }
    setScore(0)
    setGameOver(false)
  }, [])

  useEffect(() => { init(initialDir) }, []) // eslint-disable-line react-hooks/exhaustive-deps
    ...
    const interval = setInterval(() => {
      ...
      if (s.alive && s.started) {
        ...
        if (head.x < 0 || head.x >= COLS || head.y < 0 || head.y >= ROWS) {
          s.alive = false
          setGameOver(true)
          if (score > highScore) { setHighScore(score); localStorage.setItem('snake_high', score.toString()) }
          return
        }
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if (gameOver && e.key === ' ') { init('RIGHT'); return }
  ...
  return (
    <div className="relative">
      <div className="flex items-center justify-between px-2 h-7" style={{ width: W }}>
        <span className="text-xs font-bold text-accent">{score}점</span>
        <span className="text-[10px] text-text-secondary">최고 {highScore}</span>
        <button onClick={onClose} className="text-[10px] text-text-secondary hover:text-text-primary cursor-pointer">ESC 닫기</button>
      </div>
      <canvas ref={canvasRef} width={W} height={H} style={{ imageRendering: 'pixelated', display: 'block' }} />
      {gameOver && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/50 rounded-xl">
          <div className="text-center">
            <p className="text-sm font-bold text-white">Game Over! ({score}점)</p>
            <p className="text-xs text-white/60 mt-1">SPACE 재시작 / ESC 닫기</p>
          </div>
        </div>
      )}
    </div>
