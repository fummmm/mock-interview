 ...
 * 꼬리질문 생성 - 답변이 부족할 때만
 */
export async function generateFollowUp(questionText, roughTranscript) {
  if (!roughTranscript || roughTranscript.trim().length < 5) {
    return { needed: true, question: '답변을 조금 더 구체적으로 말씀해주시겠어요?', reason: '답변이 감지되지 않음' }
  }
  ...
  const content = await callOpenRouter({
    ...
    messages: [
      {
        role: 'system',
        content: `당신은 면접관입니다. 지원자의 답변을 듣고, 꼬리질문이 필요한지 판단합니다.

꼬리질문이 필요한 경우:
- 답변이 1~2문장으로 너무 짧을 때
- 구체적 사례/경험 없이 추상적으로만 답변했을 때
- 핵심 포인트(본인 역할, 결과, 교훈 등)를 빠뜨렸을 때
- "없습니다", "잘 모르겠습니다" 등 회피 답변일 때

꼬리질문이 불필요한 경우:
- 구체적 사례 + 본인 역할 + 결과/교훈까지 포함된 충분한 답변

꼬리질문 작성 원칙:
- 자연스러운 면접관 말투 ("~해주시겠어요?", "~에 대해 좀 더 말씀해주세요")
- 1문장, 짧고 명확하게
- 답변에서 부족한 부분을 정확히 파고드는 질문

반드시 JSON으로만 응답:
{ "needed": true, "question": "꼬리질문", "reason": "판단 이유" }
또는
{ "needed": false }`
      },
      {
...
## 엄격한 채점 원칙
- 답변 거부/회피("없습니다", "잘 모르겠습니다", "생각해본 적 없습니다" 등): 모든 항목 10점 이하, pass=false
- 한두 문장으로 끝나는 극히 짧은 답변: 최대 30점
- 질문과 무관한 답변: relevance 10점 이하
- 구체적 사례 없이 추상적으로만 답변: specificity 30점 이하
- 관대하게 점수를 주지 마세요. 실제 면접처럼 엄격하게 평가하세요.
- 70점 이상은 "잘 답변했다"는 의미입니다. 쉽게 주지 마세요.
...
function getEvaluatorConfig(track, trackLabel) {
  ...
  if (track === 'behavioral') {
   ...
   - 솔직하고 직설적인 피드백. 실무에서 이 답변이 통할지 냉정하게 판단.
   - "현업에서는 이런 답변은 ___해서 아쉽다" 식의 실무 맥락 피드백.

2. **면접 코치** (커리어 코칭 전문가, 면접 컨설턴트 경력 10년+)
   - 답변 구조, 전달력, 표현력, 면접 기본기 위주 평가
   - 채점은 엄격하되, 피드백은 격려와 구체적 개선 방향 중심.
   - "이 부분은 이렇게 바꿔서 연습해보세요" 식의 코칭형 피드백.
   - 수강생이 다음 연습에서 바로 적용할 수 있는 실천적 조언 제시.
...
3. **임원 면접관** (이사/본부장급, 경력 15년+)
  ...
  return {
    prompt: `1. **실무 전문가** (${trackLabel} 시니어, 경력 7년+)
   - 기술적 정확성, 실무 적용 가능성, 문제 해결 역량 위주 평가
   - 솔직하고 직설적인 피드백. "현업에서는 ___" 맥락의 실무 관점.
