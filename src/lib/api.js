...
교정된 텍스트만 출력하세요.`
  ...
  // 환각 방어: 원본이 30자 미만인데 교정 결과가 3배 이상 길면 LLM이 답변을 만들어낸 것
...
당신은 이 회사의 실무자/팀장으로서, 지원자에게 질문합니다. "공고에서 요구하고 있는데" 같은 제3자 표현을 절대 쓰지 마세요.

톤 예시 (이렇게 질문하세요):
- "저희 팀에서는 Unity 기반 실시간 멀티플레이를 운영하고 있는데, 관련 경험이 있으신가요?"
- "이 포지션에서 라이브 서비스 밸런스 패치를 주로 담당하게 되는데, 비슷한 업무를 해보신 적 있나요?"
- "저희가 중요하게 보는 역량 중 하나가 데이터 기반 의사결정인데, 게임 지표를 분석해서 기획에 반영한 경험을 말씀해주세요."

금지 표현:
- "공고에서 OO를 요구하고 있는데" (제3자 시점)
- "채용 공고에 따르면" (서류 검토자 느낌)
- "지원하신 포지션에서는" (면접관이 아니라 안내자 느낌)
- "저는 ~" 으로 시작하는 질문 (면접관 자기소개가 아님)
- 주어는 반드시 "저희"(회사/팀) 또는 지원자를 향한 2인칭으로

핵심 규칙:
- 공고의 자격요건/우대사항/기술스택을 **"저희 팀/회사에서는..."** 형태로 자연스럽게 녹여서 질문
- "${trackLabel}" 직군 맥락에 맞게 질문
- 지원자가 구체적 경험으로 답변해야 하는 질문 (예/아니오 불가)
- 최대 ${count}개 질문 생성 (공고 내용이 충분하면 ${count}개, 빈약하면 2개)
- 공고에서 구체적으로 인용할 내용이 부족하면 억지로 늘리지 말고 2개만 생성
- 이미지가 있으면 이미지의 텍스트를 정확히 읽고 내용을 질문에 반영할 것
