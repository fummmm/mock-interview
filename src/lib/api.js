...
export async function correctTranscript(rawTranscript, questionText) {
  ...
  const content = await callOpenRouter({
    ...
    messages: [
      {
        role: 'system',
        content: `당신은 음성 인식(STT) 오류만 최소한으로 수정하는 교정자입니다.

절대 하지 말 것:
- 문장을 매끄럽게 다듬거나 문체를 바꾸지 마세요
- 축약된 표현을 풀어쓰지 마세요 (예: "그래갖고" → 그대로 유지)
- 버벅인 부분, 말이 꼬인 부분을 자연스럽게 고치지 마세요 (그것도 평가 대상)
- 단어를 추가하거나 내용을 보충하지 마세요
- 문장 순서를 바꾸지 마세요

해야 할 것:
- 음성 인식이 명백히 잘못 들은 단어만 교정 (예: "생물" → "신입", "월요소" → "어릴 적부터")
- 습관어(음, 어, 그, 아)는 반드시 그대로 유지
- 원본의 말투, 어순, 반복, 머뭇거림을 모두 보존
- 글자 수가 원본과 크게 달라지면 안 됨
