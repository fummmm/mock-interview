 ...
 * 꼬리질문 생성 - 답변이 부족할 때만
 */
export async function generateFollowUp(questionText, roughTranscript, evaluatorNames = []) {
  if (!roughTranscript || roughTranscript.trim().length < 5) {
    return { needed: true, question: '답변을 조금 더 구체적으로 말씀해주시겠어요?', reason: '답변이 감지되지 않음', evaluatorId: evaluatorNames[0]?.id || 'coach' }
  }
...
꼬리질문 작성 원칙:
- 해당 면접관의 말투와 관점에 맞는 질문
- 1문장, 짧고 명확하게
- 답변에서 부족한 부분을 해당 면접관의 관점으로 파고드는 질문
