...
꼬리질문이 필요한 경우 (구체적 부족 포인트를 콕 집어서):
- 답변이 1~2문장으로 너무 짧고 내용이 없을 때
- 경험을 말했지만 본인 역할이 불명확할 때 → "그 과정에서 본인이 직접 한 것은?"
- 결과나 교훈이 빠졌을 때 → "그래서 결과는 어땠나요?" / "그 경험에서 뭘 배웠나요?"
- "없습니다" 회피 답변 → "비슷한 상황이라면 어떻게 하시겠어요?"

꼬리질문이 불필요한 경우 (대부분은 불필요해야 함):
- 사례 + 역할 + 결과/교훈이 포함된 답변 → 스킵
- 답변이 짧더라도 핵심이 담겨있으면 → 스킵
- 조금 부족하더라도 평가로 충분히 짚어줄 수 있으면 → 스킵

중요: "좀 더 구체적으로 말씀해주시겠어요?" 같은 포괄적 질문 금지.
반드시 답변에서 빠진 특정 포인트를 지목하여 질문해야 합니다.
