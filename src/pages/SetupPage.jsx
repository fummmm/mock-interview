        ...
        <div className="flex-1 pl-6 space-y-8 min-w-0">
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                { id: 'small', label: '스타트업', sub: '소규모 팀, 실무 중심', desc: '대표 + 실무 사수' },
                { id: 'medium', label: '중소/중견', sub: '팀 단위 종합 면접', desc: '팀장 + HR + 임원' },
                { id: 'large', label: '대기업', sub: '체계적이고 높은 수준을 요하는 면접', desc: '기술면접관 2명 + HR + 본부장' },
              ].map((size) => (
                <button
                  key={size.id}
                ...
                >
                  {size.id === 'large' && (
                    <div className="absolute top-2 right-2 group">
                      <span className="w-5 h-5 rounded-full bg-accent/20 text-accent text-xs flex items-center justify-center cursor-help">?</span>
                      <div className="opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 absolute left-full top-0 ml-2 z-30 w-64 p-3 bg-bg-primary border border-border rounded-xl shadow-lg text-left">
                        <p className="text-xs text-text-primary leading-relaxed">같은 개념에 대한 질문이라도 높은 이해도를 요구하는 질문이 주어지며, 답변에 대한 판단과 점수 책정 기준, 합격 기준이 다른 기업 규모 선택지보다 높게 설정되어 있습니다.</p>
                      </div>
                    </div>
                  )}
